...
function showTab(n) {
  ...
  if (n == x.length - 1) {
	...
	window.pencolor = "black"; // set the black color for signature
  } else {
...
function takeScreenShotOfHTML() {
  ...
  if (NUM == 1) {
    html2canvas($("#sheet")[0], opt).then(function (canvas) {
      console.log("checking=>", canvas);
      window.bodyImg = canvas.toDataURL("image/jpeg", 1.0);
      console.log("checkingbodyIMG=>", window.bodyImg);
    });
  }
  
  console.log("docu log",document.getElementById("sheet").toDataURL("image/png"));
